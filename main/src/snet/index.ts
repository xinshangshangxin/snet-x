...
import { access, createWriteStream, ensureFile, writeJSON } from 'fs-extra';
...
class Snet {
  private snetPath: string = resolve(__dirname, '../../assets/snet');
  ...
  public async checkSnetBin() {
    if (!this.snetPath) {
      throw new Errors.NoSnetPath();
    }

    try {
      await access(this.snetPath);
    } catch (e) {
      throw new Errors.NoSnetFound();
    }
  }
}
